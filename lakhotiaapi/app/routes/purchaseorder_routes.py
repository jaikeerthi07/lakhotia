...
def create_purchase_order():
    try:
        ...
        for item in items:
            ...
            mrp = float(item.get('mrp', buy_price))
            
            # Validation: Buy Price (calculated piece price) should be less than MRP * L * W
            # If length/width are present, we should compare the unit price
            buy_price_unit = float(item.get('buy_price_unit', 0))
            mrp_unit = float(item.get('mrp', 0))
            if buy_price_unit > 0 and mrp_unit > 0 and buy_price_unit >= mrp_unit:
                return jsonify({
                    'success': False,
                    'error': f'Buy Price ({buy_price_unit}) must be less than MRP ({mrp_unit}) for item {item.get("item_name")}'
                }), 400
    ...
    try:
        ...
        if 'items' in data:
            ...
            for i in data['items']:
                ...
                bp = float(i.get('buy_price', 0))
                bp_unit = float(i.get('buy_price_unit', 0))
                m_unit = float(i.get('mrp', 0))
                
                if bp_unit > 0 and m_unit > 0 and bp_unit >= m_unit:
                    return jsonify({
                        'success': False,
                        'error': f'Buy Price ({bp_unit}) must be less than MRP ({m_unit}) for item {i.get("item_name")}'
                    }), 400
