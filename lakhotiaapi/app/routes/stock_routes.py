...
def check_batch_unique():
    ...
    try:
        ...
        for item in items:
            ...
            if batch_code:
                # If batch code is provided, it MUST be unique globally
                stock_item = Stock.query.filter(Stock.batch_code == batch_code).first()
            else:
...
def bulk_save_stock():
    ...
    try:
        ...
        for r in records:
            ...
            # CRITICAL: Batch Code must be globally unique
            if batch_code:
                existing_stock = Stock.query.filter(Stock.batch_code == batch_code).first()
                if existing_stock and existing_stock.item_name != item_name:
                    duplicate_details.append({
                        "item_name": item_name,
                        "batch_code": batch_code,
                        "reason": f"Batch code used by another item: {existing_stock.item_name}"
                    })
                    skipped.append(f"{item_name} (Batch {batch_code} used by {existing_stock.item_name})")
                    continue
            
            # If no batch code, try to find item with same name and no batch code
            if not existing_stock and item_name and not batch_code:
            ...
            if existing_stock:
                ...
                if r.get("item_type"):
                    existing_stock.item_type = r.get("item_type", existing_stock.item_type).strip()
                if r.get("status"):
                    existing_stock.status = r.get("status", existing_stock.status).strip()
            ...
            new_stock = Stock(
                ...
                item_type=r.get("item_type", "stock").strip(),
        ...
        # Check if it's a duplicate batch code error
        error_msg = str(e)
        if "Duplicate entry" in error_msg and "batch_code" in error_msg:
            return jsonify({
                "success": False, 
                "message": "One or more items have a Batch Code that already exists. Each Batch Code must be unique."
            }), 400
...
def update_stock_bulk():
    ...
    try:
        ...
        for r in records:
            ...
            if "item_type" in r:
                stock.item_type = r.get("item_type", stock.item_type).strip()
            if "status" in r:
                stock.status = r.get("status", stock.status).strip()
        ...
        if "Duplicate entry" in error_msg and "batch_code" in error_msg:
            return jsonify({
                "success": False, 
                "message": "Update failed: A Batch Code you entered already exists for another item."
            }), 400
