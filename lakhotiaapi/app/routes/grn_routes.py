...
def get_all_pos_with_status():
    try:
        ...
        all_pos = PurchaseOrder.query.filter(
            PurchaseOrder.status.in_(['completed', 'approved'])
                ...
                if waiting_for_grn > 0:
                    remaining_items.append({
                        **item,
                        ...
                        'grn_delivered_quantity': delivered_qty,
                        'od_received_quantity': od_qty,
                        'available_to_grn': waiting_for_grn,
                        # Compatibility field for older frontend versions
                        'delivered_quantity': waiting_for_grn 
                    })
    ...
    try:
        ...
        for item in po.items:
            ...
            enhanced_item = {
                **item,
                'ordered_quantity': ordered_qty,
                'grn_delivered_quantity': grn_delivered_qty,
                'od_received_quantity': od_received_qty,
                'available_to_grn': available_for_grn,
                # Compatibility: Set delivered_quantity so old code still works
