...
export default function StockUploadPage() {
  ...
  const findMatchingStockItemInRows = (soldItem) => {
    ...
    for (const row of rows) {
      ...
      // If sold item has batch code, we must match by batch code only
      if (batchCode) {
        if (rowBatchCode === batchCode) {
          match = true;
        }
      } else {
        // No batch code on sold item, match by Item Name as fallback
  ...
  const isItemWithBatchExists = (itemName, batchCode, existingRows) => {
    ...
    if (normalizedBatchCode) {
      return existingRows.some(row => (row["Batch Code"] || "").toLowerCase().trim() === normalizedBatchCode);
    }
    
    // If no batch code, fallback to Item Name
    const normalizedItemName = (itemName || "").toLowerCase().trim();
    return existingRows.some(row => (row["Item Name"] || "").toLowerCase().trim() === normalizedItemName && !(row["Batch Code"] || "").trim());
  };
