...
const PurchaseOrderPage = () => {
  ...
  const handleItemTyping = (e) => {
    ...
    const results = stockData.filter((x) =>
      ...
      x["Brand"]?.toLowerCase().includes(value.toLowerCase()) ||
      x["brand_code"]?.toLowerCase().includes(value.toLowerCase()) ||
      x["Brand Code"]?.toLowerCase().includes(value.toLowerCase()) ||
      x["hsn_code"]?.includes(value) ||
      x["HSN"]?.includes(value)) &&
      x.status !== 'old' // Allow searching all active items for templates
    );
  ...
  const handleSuggestionSelect = (item) => {
    ...
    if (length > 0 && width > 0) {
      ...
      // Using Buy Price per unit for calculation
      buyPrice = (parseFloat(item["Buy Price"] || item.buy_price || 0)) * length * width;
    }
    ...
    setFormData({
      ...
      buy_price_unit: parseFloat(item["Buy Price"] || item.buy_price || 0),
  ...
  const handleChange = (e) => {
    ...
    // Recalculate buy_price when length, width, or buy_price_unit changes
    if (name === 'length' || name === 'width' || name === 'buy_price_unit') {
      const length = parseFloat(name === 'length' ? value : formData.length) || 0;
      const width = parseFloat(name === 'width' ? value : formData.width) || 0;
      const bpUnit = parseFloat(name === 'buy_price_unit' ? value : formData.buy_price_unit) || 0;
      
      if (length > 0 && width > 0 && bpUnit > 0) {
        // Price/Unit = Buy Price (Unit) × Length × Width
        const calculatedPrice = (bpUnit * length * width).toFixed(2);
        newFormData.buy_price = calculatedPrice;
      } else if (bpUnit > 0) {
        newFormData.buy_price = bpUnit.toFixed(2);
      }
    }
  ...
  const calculatePricePerUnit = (bpUnit, length, width) => {
    const b = parseFloat(bpUnit) || 0;
    const l = parseFloat(length) || 0;
    const w = parseFloat(width) || 0;
    
    if (b > 0 && l > 0 && w > 0) {
      return (b * l * w).toFixed(2);
    }
    return b > 0 ? b.toFixed(2) : "0.00";
  };

  // Add item to temporary list
  const handleAddItem = () => {
    if (!formData.item_name || !formData.quantity || !formData.buy_price || !formData.mrp || !formData.buy_price_unit) {
      toast.error("Item Name, Quantity, Buy Price, Buy Price Unit, and MRP are required!");
      return;
    }

    if (parseFloat(formData.buy_price_unit) >= parseFloat(formData.mrp)) {
      toast.error("Buy Price (Per Unit) must be less than MRP!");
      return;
    }

    // Removed validation that blocked items with dimensions
    ...
    if (length > 0 && width > 0) {
      // Count = Length × Width × Quantity
      count = length * width * quantity;
      // Note: buyPrice already includes L*W from handleChange
    }
    ...
    const itemWithId = {
      ...
      unit: formData.unit,
      quantity: quantity,
      buy_price_unit: parseFloat(formData.buy_price_unit),
      buy_price: buyPrice,
      mrp: mrp,
      count: count,
  ...
  const handleSubmitPO = async () => {
    ...
    try {
      ...
      if (editMode && currentPoId) {
        // Update existing PO
        const response = await axios.put(`${API_BASE_URL}/update/${currentPoId}`, payload);
            ...
            <div className="modal-content">
                ...
                <div className="card mb-4">
                  ...
                  <div className="card-body">
                    <div className="row g-3 mb-3">
                      ...
                      <div className="col-md-2">
                        <label className="form-label fw-bold">Buy Price (Per Unit) *</label>
                        <div className="input-group">
                      ...
                      <div className="col-md-2">
                        ...
                        <div className="input-group">
                          ...
                          <input
                            ...
                            step="0.01"
                            required
                            readOnly={formData.length && formData.width && formData.buy_price_unit}
                          />
                        </div>
                        <small className="text-muted">
                          {formData.length && formData.width && formData.buy_price_unit ? 
                            `Calculated: BP Unit (${formData.buy_price_unit}) × L (${formData.length}) × W (${formData.width})` : 
                            "Enter BP Unit, Length & Width to auto-calculate"}
                        </small>
                      </div>
                      ...
                      <div className="alert alert-info mb-3">
                        <div className="row">
                          ...
                          <div className="col-md-4">
                            ...
                            <div>Buy Price (Unit) × Length × Width</div>
                            {formData.buy_price_unit && (
                              <>
                                <div>= {formData.buy_price_unit} × {formData.length} × {formData.width}</div>
                                <div className="fw-bold">
                                  = ₹{calculatePricePerUnit(formData.buy_price_unit, formData.length, formData.width)}
                                </div>
                              </>
                            )}
                          </div>
              ...
              <div className="modal-footer">
                <button
                  ...
                  disabled={submitting || tempItems.length === 0 || !formData.company_name || !formData.customer_name}
                >
                  {submitting ? (
                    <>
