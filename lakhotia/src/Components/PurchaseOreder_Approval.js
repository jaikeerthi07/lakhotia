...
const PurchaseOrderPage = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'length' || name === 'width' || name === 'mrp') {
      ...
      console.log(`Calculation trigger: ${name}=${value}, L=${length}, W=${width}, MRP=${mrp}`);
      
      if (length > 0 && width > 0 && mrp > 0) {
        // Price/Unit = MRP × Length × Width
        const calculatedPrice = (mrp * length * width).toFixed(2);
        console.log(`Calculated Buy Price (Dimensions): ${calculatedPrice}`);
        newFormData.buy_price = calculatedPrice;
      } else if (mrp > 0) {
        // If no dimensions, use MRP as buy price
        console.log(`Using MRP as Buy Price: ${mrp.toFixed(2)}`);
  ...
  const handleSubmitPO = async () => {
    ...
    try {
      ...
      console.log("Submitting PO payload (Approval Component):", payload);
