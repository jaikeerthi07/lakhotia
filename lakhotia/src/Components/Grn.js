...
const GRNPage = () => {
  ...
  // Handle PO selection from buttons - FIXED: Force fresh data fetch for reliability
  const handlePOSelect = (poNum) => {
    setPoNumber(poNum);
    fetchPODetails(poNum); // Don't pass preloaded data to ensure robustness
  };
  ...
  const fetchPODetails = async (poNum = null, preloadedData = null) => {
    ...
    try {
      ...
      if (poData.items && poData.items.length > 0) {
        // Filter items that have been received (available to GRN)
        receivedItems = poData.items.filter(item => 
          (item.available_to_grn || item.delivered_quantity || 0) > 0
        );
      }
      ...
      if (receivedItems && receivedItems.length > 0) {
        ...
        const itemsWithBatch = receivedItems.map((item, index) => ({
          ...
          index: index,
          received_quantity: item.available_to_grn || item.delivered_quantity || 0,
          original_quantity: item.ordered_quantity || item.original_quantity || item.quantity,
