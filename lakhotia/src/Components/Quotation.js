...
export default function QuotationModal() {
  ...
  function createEmptyItem(seq) {
    ...
    return {
      ...
      tax_rate: 18.0,
      packing_charges: 0,
      freight_charges: 0, // Renamed from other_charges
      item_status: "pending",
      customer_description: "",
      brand_code: "",
      profit_percentage: 20 // Each item has its own profit percentage
    };
  }
  ...
  const [newItemFreightCharges, setNewItemFreightCharges] = useState("0"); // Renamed
  ...
  const [newItemProfitPercentage, setNewItemProfitPercentage] = useState("20"); // Per item profit
  const [newItemBuyPrice, setNewItemBuyPrice] = useState("0");
  const [newItemMrp, setNewItemMrp] = useState("0");
  ...
  // Edit existing item using popup
  function editItemViaPopup(index) {
    const item = items[index];
    setEditingItemIndex(index);
    
    // Create a mock stock item to satisfy the popup's dependencies
    setSelectedStockItem({
      id: item.stock_id || Date.now(),
      ...
      "Unit": item.unit,
      "Batch Code": item.batch_no,
      "Brand Description": item.description,
      "HSN": item.hsn_sac
    });
    
    setItemSearchTerm(item.item_name);
    setNewItemCutWidth(item.cut_width.toString());
    setNewItemLength(item.length.toString());
    setNewItemQuantity(item.quantity.toString());
    setNewItemBatchCode(item.batch_no);
    setNewItemDiscount(item.discount.toString());
    setNewItemDiscountType(item.discount_type);
    setNewItemPackingCharges(item.packing_charges.toString());
    setNewItemFreightCharges(item.freight_charges.toString());
    setNewItemCustomerDescription(item.customer_description);
    setNewItemSupplierPartNo(item.supplier_part_no);
    setNewItemBrandCode(item.brand_code);
    setNewItemProfitPercentage(item.profit_percentage.toString());
    setNewItemBuyPrice(item.buy_price.toString());
    setNewItemMrp(item.mrp.toString());
  ...
  // Calculate total with profit for an item: Price/Unit + (Price/Unit × profit%)
  const calculateTotalWithProfit = (item) => {
    const pricePerUnitValue = pricePerUnit(item);
    const profitPercent = parseFloat(item.profit_percentage) || 20;
    const profit = pricePerUnitValue * (profitPercent / 100);
                        ...
                        <table className="table table-hover mb-0">
                          <thead className="table-light">
                            <tr>
                              ...
                              <th width="80">Qty</th>
                              <th width="80">Unit</th>
                              <th width="110">Buy Price</th>
                              <th width="110">Sell Price (MRP)</th>
                                ...
                                <td>
                                  <div className="d-flex gap-1">
                                    <button
                                      ...
                                      onClick={() => editItemViaPopup(index)}
                                      title="Edit"
                                    >
                        ...
                        <tbody>
                          {items.map((item, index) => (
                            <tr key={item.id}>
                              ...
                              <td>{item.profit_percentage || 20}%</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              <div className="modal-body">
                <div className="mb-3">
                  ...
                  <input
                    ...
                    onFocus={() => {
                      const focused = itemSearchTerm.trim()
                        ? stockItems.filter(item =>
                            item["Item Name"]?.toLowerCase().includes(itemSearchTerm.toLowerCase()) ||
                            item["Brand Code"]?.toLowerCase().includes(itemSearchTerm.toLowerCase()) ||
                            item["Brand Description"]?.toLowerCase().includes(itemSearchTerm.toLowerCase())
                          )
                        : stockItems;
                      setSearchResults(focused);
                      setShowResults(focused.length > 0);
                    }}
                  />
                      ...
                      {searchResults.map((item, idx) => (
                        <div
                          ...
                          onClick={() => {
                            ...
                            setNewItemMrp((item["MRP"] || 0).toString());
                            setNewItemBuyPrice((item["Buy Price"] || 0).toString());
                            setShowResults(false);
                  ...
                  <div className="card mt-3 mobile-card">
                    ...
                    <div className="card-body">
                      ...
                      <div className="row mobile-row">
                        ...
                        <div className="col-md-3">
                          <div className="mb-3">
                            <label className="form-label">Sell Price (MRP)</label>
                            <input
                              type="number"
                              className="form-control"
                              step="0.01"
                        ...
                        <div className="col-md-4">
                          <div className="mb-3">
                            ...
                            <input 
                              type="text" 
                              className="form-control" 
                              value={"₹" + ((parseFloat(newItemMrp) || 0) * (parseFloat(newItemLength) || 0) * (parseFloat(newItemCutWidth) || 0) || 0).toFixed(2)} 
                              readOnly 
                            />
                    ...
                    <div className="card-footer">
                      <div className="d-flex justify-content-end gap-2 mobile-btn-group">
                        ...
                        <button type="button" className="btn btn-primary" onClick={() => {
                          ...
                          if (parseFloat(newItemBuyPrice) >= parseFloat(newItemMrp)) {
                            alert("Buy Price must be less than Sell Price (MRP)!");
                            return;
                          }
                          
                          const itemData = {
                            id: editingItemIndex !== null ? items[editingItemIndex].id : (idRef.current + 1),
                            ...
                            cut_width: parseFloat(newItemCutWidth) || 1,
                            length: parseFloat(newItemLength) || 1,
                            ...
                            buy_price: parseFloat(newItemBuyPrice) || 0,
                            ...
                            profit_percentage: parseFloat(newItemProfitPercentage) || 20,
                            item_status: "pending"
                          };
                          ...
                          <i className={`bi ${editingItemIndex !== null ? 'bi-save' : 'bi-plus-circle'} me-1`}></i>
                          {editingItemIndex !== null ? 'Update Item' : 'Add to Quotation'}
                        </button>
                      </div>
                    </div>
                  </div>
